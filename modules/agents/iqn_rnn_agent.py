...
class IQNRNNAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state, forward_type=None):
        ...
        if forward_type == "approx":   
            n_rnd_quantiles = self.n_approx_quantiles   # 32
        elif forward_type == "policy":
            n_rnd_quantiles = self.n_quantiles          # 8
        elif forward_type == "target":
            n_rnd_quantiles = self.n_target_quantiles   # 8
        else:
        ...
        # cos embedding
        feature_id = th.arange(0, self.quantile_embed_dim).cuda() # [0,1,...,63]
