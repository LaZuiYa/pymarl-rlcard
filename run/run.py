...
def run_sequential(args, logger):
    ...
    # Default/Base scheme
    scheme = {
        "state": {"vshape": env_info["state_shape"], "dtype": "dict"},
        "obs": {"vshape": env_info["obs_shape"], "group": "agents", "dtype": "dict"},
        "actions": {"vshape": (1,), "dtype": th.long},
        "avail_actions": {"vshape": (env_info["n_actions"],), "group": "agents", "dtype": "dict"},
        "player_id": {"vshape": 1, "dtype": th.int8},
        "reward": {"vshape": (1,)},
    ...
    while runner.t_env <= args.t_max:
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            for _ in range(n_test_runs):
                runner.run(test_mode=True, model_palyer_id = [0])
            for _ in range(n_test_runs):
                runner.run(test_mode=True, model_palyer_id=[1,2])
            # for _ in range(n_test_runs):
            #     runner.run(test_mode=True, model_palyer_id=2)
