...
class RlCardActionSelector():
    ...
    def select_action(self, agent_inputs,  t_env, test_mode=False):
        # Assuming agent_inputs is a batch of Q-Values for each agent bav
        self.epsilon = self.schedule.eval(t_env)
        batch_size = len(agent_inputs)
        max_len = max(len(x) for x in agent_inputs)

        # 构造 mask 和 pad 成 tensor
        padded = th.ones(batch_size, max_len)
        padded = padded*-9999
        valid_actions = th.zeros_like(padded)
        for i, acts in enumerate(agent_inputs):
            padded[i, :len(acts)] = acts.squeeze(-1)
            valid_actions[i, :len(acts)] = 1

        if test_mode:
            # 直接选最大值
            chosen_actions = padded.max(dim=1)[1].cpu().numpy()
        else:
            # 根据 epsilon 选动作
            batch_eps = th.tensor([self.epsilon] * batch_size).unsqueeze(-1)
            random_prob = th.rand(batch_size, 1)
            pick_random = (random_prob < batch_eps).float().cpu()
            random_actions = th.multinomial(valid_actions, 1).cpu()
            greedy_actions = padded.max(dim=1)[1].unsqueeze(-1).cpu()
            chosen_actions = (
                        pick_random * random_actions + (1 - pick_random) * greedy_actions).long().squeeze().numpy()

        return chosen_actions


REGISTRY["rlcard"] = RlCardActionSelector