...
class EpisodeBatch:
    ...
    def _setup_data(self, scheme, groups, batch_size, max_seq_length, preprocess):
        ...
        for field_key, field_info in scheme.items():
            ...
            if episode_const:
                if dtype == "dict":
                    self.data.episode_data[field_key] = [{} for _ in range(batch_size)]
                else:
                    self.data.episode_data[field_key] = th.zeros((batch_size, *shape), dtype=dtype, device=self.device)
            else:
                if dtype == "dict":
                    self.data.transition_data[field_key] = [[{}] * max_seq_length for _ in range(batch_size)]
                else:
    ...
    def assign_target_with_slices(self, target_k, v, slices):
        """
        Args:
            target_k: list of lists, shape: (batch_size, max_seq_length, ...)
            v: 要赋的值，可以是 list 或 tensor
            slices: tuple of slices, e.g. (slice(None), slice(0, 1))
        """
        batch_slice, time_slice = slices

        # Step 1: 处理 batch slice
        if isinstance(batch_slice, slice):
            if batch_slice == slice(None):  # 全选 batch
                batch_indices = range(len(target_k))
            else:
                # 解析 slice(start, stop, step) 成具体 index
                batch_indices = range(*batch_slice.indices(len(target_k)))
        elif isinstance(batch_slice, (list, np.ndarray)):
            batch_indices = batch_slice
        else:
            batch_indices = [batch_slice]

            # Step 2: 处理 time slice
        if isinstance(time_slice, slice):
            time_indices =  range(*time_slice.indices(len(target_k[0])))
        ...
        for i, b in enumerate(batch_indices):
            for t in time_indices:
                target_k[b][t] = v[i] if len(time_indices) == 1 else v[i][t]
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            if dtype == "dict":
                    self.assign_target_with_slices(target[k], v, _slices)

            else:
                v = th.tensor(v, dtype=dtype, device=self.device)

                self._check_safe_view(v.view_as(target[k][_slices]), target[k][_slices])
    ...
    def __getitem__(self, item):
        if isinstance(item, str):
            ...
            new_data = self._new_data_sn()
            for k, v in self.data.transition_data.items():
                if k in ['obs', 'state', 'avail_actions'] :
                    new_data.transition_data[k] = self.nested_list_getter(v, item)
                else:
                    # 确保是 tensor
                    tensor_v = v.to('cuda') if isinstance(v, th.Tensor) else th.tensor(v, device='cuda')
                    new_data.transition_data[k] = tensor_v[item]
            for k, v in self.data.episode_data.items():
    ...
    def nested_list_getter(self, data, item):
        """
        支持对嵌套 list 使用类似 numpy 的复合切片
        :param data: list of lists，比如 shape: (batch, time, ...)
        :param item: tuple of slices，如 (slice(None), slice(0, 1))
        :return: 提取后的 list
        """
        batch_slice, time_slice = item
        result = []

        # 解析 batch 维度
        if isinstance(batch_slice, slice):
            batch_indices = range(*batch_slice.indices(len(data)))
        elif isinstance(batch_slice, (list, np.ndarray)):
            batch_indices = batch_slice
        else:
            batch_indices = [batch_slice]

        # 提取每个 batch 的 time_slice 内容
        for b in batch_indices:
            if isinstance(time_slice, slice):
                result.append(data[b][time_slice])
            else:
                result.append(data[b][time_slice])
