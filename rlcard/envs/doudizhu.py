...
class DoudizhuEnv(Env):
    ...
    def _extract_state(self, state):
        ...
        last_2_actions = _action_seq2array(_process_action_seq(state['trace']))
        ...
        if state['self'] == 0: # landlord
            ...
            landlord_up_num_cards_left = _get_one_hot_array(state['num_cards_left'][2], 20)
            landlord_down_num_cards_left = _get_one_hot_array(state['num_cards_left'][1], 20)
            obs = np.concatenate(([1,0,0],
                                  current_hand,
                                  others_hand,
                                  last_2_actions,
            ...
            teammate_num_cards_left = _get_one_hot_array(state['num_cards_left'][teammate_id], 20)
            obs = np.concatenate((np.eye(3)[state['self']],
                                  current_hand,
                                  others_hand,
                                  last_2_actions,
...
def _process_action_seq(sequence, length=2):
