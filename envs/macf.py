...
class MultiAgentCar(MultiAgentEnv):
    def __init__(self, batch_size=None, **kwargs):
        ...
        if self.map_name in ["macf_opt"]:
            ...
            self.crash_reward = -1  # -1
            self.termination_reward = 20  # 10
            self.num_fails = 0
            self.num_reached = 0
            self.over_speeds = 0
        ...
        for i in range(self.n_agents):
            if self.reached[i] == False:
                ...
                if (self.vel[i] + accel * self.delta_time_per_step) >= self.speed_max:
                    ...
                    self.pos[i] = self.pos[i] + self.vel[
                        i] * self.delta_time_per_step + 0.5 * accel * self.delta_time_per_step * self.delta_time_per_step
